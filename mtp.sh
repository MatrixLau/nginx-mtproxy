...
read -rp "你需要TAG标签吗(Y/N): " chrony_install
    ...
    [yY][eE][sS] | [yY])
        ...
        docker run --name nginx-mtproxy -d -e tag="$tag" -e secret="$secret" -e domain="$domain" -e ip_white_list="$iplist" -p $nport:80 -p $port:8443 ellermister/nginx-mtproxy:latest
        ;;
    *)
...
echo -e "正在安装nginx-mtproxy... "
docker run --name nginx-mtproxy -d -e secret="$secret" -e domain="$domain" -e ip_white_list="$iplist" -p $nport:80 -p $port:8443 ellermister/nginx-mtproxy:latest
        ;;
    esac
